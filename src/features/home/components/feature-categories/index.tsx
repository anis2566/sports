...
import { useGetFeatureCategories } from "../../api/use-get-feature-categories";

export const FeatureCategories = () => {
    const { data, isLoading } = useGetFeatureCategories();

    return (
        <div className="px-3 md:px-0 space-y-2">
            <h2 className="text-2xl font-bold text-gray-600">Featured Categories</h2>
            <div>
                <Carousel
                    opts={{
                        align: "start",
                    ...
                    <CarouselContent>
                        {
                            isLoading ? (
                                ...
                                data?.categories.map((category) => (
                                    <CarouselItem key={category.id} className="basis-1/2 md:basis-1/6 rounded-full">
